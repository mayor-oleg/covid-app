...
def parsercovid(d=None):
    ...
    while today > dates:
        ...
        print ('loading data from: ' , ((dates+timedelta(z)).strftime("%m-%d-%Y")))
        ...
        df = db.drop(db.loc[db['State'] == 'Recovered'].index, inplace=True)
        df = pd.DataFrame(db[['Country','Confirmed','Recovered','Deaths','Active']])
        df.fillna(0)
        #print(df)
        z+=1
        yield df.fillna(0)
